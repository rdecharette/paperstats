...
print("Count authors affiliations")
plt.figure()
s = aff_u_c.argsort()[::-1][:25]
sns.barplot(aff_u_c[s], edgecolor="black")
plt.xticks(range(len(aff_u_c[s])), [s if len(s) < 35+5 else s[:35]+" (..)" for s in aff_u[s]], rotation=45, ha='right')
...
print("Count French authors affiliations")
...
plt.figure()
s = aff_fr_u_c.argsort()[::-1][:25]
sns.barplot(aff_fr_u_c[s], edgecolor="black")
plt.xticks(range(len(aff_fr_u_c[s])), [s if len(s) < 35+5 else s[:35]+" (..)" for s in aff_fr_u[s]], rotation=45, ha='right')
...
print("Count paper w/ French affiliations")
